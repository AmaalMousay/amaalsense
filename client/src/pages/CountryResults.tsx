...
const COUNTRY_NAMES: Record<string, { en: string; ar: string }> = {
  LY: { en: "Libya", ar: "" },
  EG: { en: "Egypt", ar: "" },
  SA: { en: "Saudi Arabia", ar: "" },
  AE: { en: "UAE", ar: "" },
  US: { en: "United States", ar: " " },
  GB: { en: "United Kingdom", ar: " " },
  FR: { en: "France", ar: "" },
  DE: { en: "Germany", ar: "" },
  JP: { en: "Japan", ar: "" },
  CN: { en: "China", ar: "" },
  ...
  CA: { en: "Canada", ar: "" },
  AU: { en: "Australia", ar: "" },
  KR: { en: "South Korea", ar: " " },
  MX: { en: "Mexico", ar: "" },
  ZA: { en: "South Africa", ar: " " },
  NG: { en: "Nigeria", ar: "" },
  KE: { en: "Kenya", ar: "" },
  ID: { en: "Indonesia", ar: "" },
  PK: { en: "Pakistan", ar: "" },
...
function getMoodStatus(gmi: number) {
  if (gmi >= 50) return { label: "Very Positive", labelAr: " ", color: "text-green-500", bgColor: "bg-green-500/10", borderColor: "border-green-500/30" };
  if (gmi >= 20) return { label: "Positive", labelAr: "", color: "text-green-400", bgColor: "bg-green-400/10", borderColor: "border-green-400/30" };
  if (gmi >= -20) return { label: "Neutral", labelAr: "Neutral", color: "text-yellow-500", bgColor: "bg-yellow-500/10", borderColor: "border-yellow-500/30" };
  if (gmi >= -50) return { label: "Negative", labelAr: "", color: "text-orange-500", bgColor: "bg-orange-500/10", borderColor: "border-orange-500/30" };
  return { label: "Very Negative", labelAr: " ", color: "text-red-500", bgColor: "bg-red-500/10", borderColor: "border-red-500/30" };
}

function getSentimentBadge(sentiment: string, isRTL: boolean) {
  switch (sentiment) {
    case 'positive':
      return <Badge className="bg-green-500/20 text-green-400 border-green-500/30"><CheckCircle className="w-3 h-3 mr-1" />{isRTL ? '' : 'Positive'}</Badge>;
    case 'negative':
      return <Badge className="bg-red-500/20 text-red-400 border-red-500/30"><XCircle className="w-3 h-3 mr-1" />{isRTL ? '' : 'Negative'}</Badge>;
    default:
      return <Badge className="bg-gray-500/20 text-gray-400 border-gray-500/30"><Minus className="w-3 h-3 mr-1" />{isRTL ? 'Neutral' : 'Neutral'}</Badge>;
  }
}
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1">
            ...
            {item.source || (isRTL ? '  ' : 'Unknown source')}
          </span>
          {item.publishedAt && (
            <span className="flex items-center gap-1">
      ...
      <div className="container py-8">
        <Card className="text-center py-12">
          <CardContent>
            ...
            <p className="text-muted-foreground mb-4">{isRTL ? '   ' : 'No country selected'}</p>
        ...
        <div className="container py-8">
          <Card className="text-center py-12">
            <CardContent>
              ...
              <h3 className="text-xl font-bold mb-2">{isRTL ? 'Error  Analysis' : 'Analysis Error'}</h3>
              <p className="text-muted-foreground mb-4">
                {isRTL ? '      .    .' : 'Could not fetch data for this country. Please try again.'}
              </p>
      ...
      <div className="container py-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card className={`border-2 ${moodStatus.borderColor}`}>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <p className="text-sm text-muted-foreground">GMI - {isRTL ? '  ' : 'Global Mood Index'}</p>
            ...
            <Card className={`border-2 ${(data?.cfi ?? 50) > 60 ? 'border-red-500/30' : (data?.cfi ?? 50) > 40 ? 'border-yellow-500/30' : 'border-green-500/30'}`}>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <p className="text-sm text-muted-foreground">CFI - {isRTL ? ' Fear ' : 'Crisis Fear Index'}</p>
                    <div className="flex items-baseline gap-2">
                ...
                <p className={`text-sm font-medium ${(data?.cfi ?? 50) > 60 ? 'text-red-500' : (data?.cfi ?? 50) > 40 ? 'text-yellow-500' : 'text-green-500'}`}>
                  {(data?.cfi ?? 50) > 60 ? (isRTL ? '' : 'High') : (data?.cfi ?? 50) > 40 ? (isRTL ? '' : 'Medium') : (isRTL ? '' : 'Low')}
                </p>
              </CardContent>
            </Card>
            ...
            <Card className={`border-2 ${(data?.hri ?? 50) > 60 ? 'border-green-500/30' : (data?.hri ?? 50) > 40 ? 'border-yellow-500/30' : 'border-red-500/30'}`}>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <p className="text-sm text-muted-foreground">HRI - {isRTL ? '  ' : 'Hope Resilience Index'}</p>
                    <div className="flex items-baseline gap-2">
                ...
                <p className={`text-sm font-medium ${(data?.hri ?? 50) > 60 ? 'text-green-500' : (data?.hri ?? 50) > 40 ? 'text-yellow-500' : 'text-red-500'}`}>
                  {(data?.hri ?? 50) > 60 ? (isRTL ? '' : 'High') : (data?.hri ?? 50) > 40 ? (isRTL ? '' : 'Medium') : (isRTL ? '' : 'Low')}
                </p>
              </CardContent>
            </Card>
          </div>
            ...
            <Card className={`${moodStatus.bgColor} border ${moodStatus.borderColor}`}>
              <CardContent className="pt-6">
                <h3 className="font-bold mb-2 flex items-center gap-2">
                  ...
                  {isRTL ? ' Analysis' : 'Analysis Summary'}
                </h3>
                ...
                <div className="flex items-center gap-4 mt-4 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1">
                    ...
                    {data?.sourceCount || allNews.length} {isRTL ? '' : 'sources'}
                  </span>
                  <span className="flex items-center gap-1">
                    <Clock className="w-3 h-3" />
                    {isRTL ? ' : ' : 'Last updated: now'}
                  </span>
                </div>
              </CardContent>
            </Card>
        ...
        <section>
          <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
            ...
            {isRTL ? ' ' : 'Real News'}
            <Badge variant="secondary">{allNews.length} {isRTL ? '' : 'articles'}</Badge>
          </h2>
            ...
            <Card className="text-center py-12">
              <CardContent>
                ...
                <p className="text-muted-foreground">
                  {isRTL ? '      ' : 'No news currently available for this country'}
                </p>
              </CardContent>
            </Card>
          ) : (
              ...
              <TabsContent value="political">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {politicalNews.length > 0 ? politicalNews.map((item: any, i: number) => (
                    ...
                    <p className="text-muted-foreground col-span-2 text-center py-8">{isRTL ? '   ' : 'No political news'}</p>
                  )}
                </div>
              </TabsContent>
              ...
              <TabsContent value="economic">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {economicNews.length > 0 ? economicNews.map((item: any, i: number) => (
                    ...
                    <p className="text-muted-foreground col-span-2 text-center py-8">{isRTL ? '   ' : 'No economic news'}</p>
                  )}
                </div>
              </TabsContent>
              ...
              <TabsContent value="social">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {socialNews.length > 0 ? socialNews.map((item: any, i: number) => (
                    ...
                    <p className="text-muted-foreground col-span-2 text-center py-8">{isRTL ? '   ' : 'No social news'}</p>
                  )}
                </div>
              </TabsContent>
            </Tabs>
        ...
        <section className="text-center pb-8">
          ...
          >
            ...
            {isRTL ? `Analysis   ${countryInfo.ar}` : `Deep Analysis for ${countryInfo.en}`}
          </Button>
        </section>
      </div>
    </div>
