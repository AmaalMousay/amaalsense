...
export function generateVoiceInstructions(
  ...
  if (preferences.tone === 'formal') {
    promptModifier += lang === 'ar' 
      ? t('auto.cognitiveEngine_personalVoice.8.b26f59ee', 'ar') 
    ...
    promptModifier += lang === 'ar'
      ? t('auto.cognitiveEngine_personalVoice.7.965e0332', 'ar')
      : "Use simple, conversational language like a friendly advisor explaining a topic simply. ";
  } else {
    // Analytical default
    promptModifier += lang === 'ar'
      ? t('auto.cognitiveEngine_personalVoice.6.12a3d7e9', 'ar')
  ...
  if (emotionalIntensity > 0.8 && ['fear', 'anger'].includes(dominantEmotion)) {
    promptModifier += lang === 'ar'
      ? t('auto.cognitiveEngine_personalVoice.5.7514ecdb', 'ar')
  ...
  if (preferences.detailLevel === 'brief') {
    formattingRules.push(lang === 'ar' ? t('auto.cognitiveEngine_personalVoice.4.ac3aadd3', 'ar') : "Keep the answer very short and direct using bullet points.");
  } else if (preferences.detailLevel === 'comprehensive') {
    formattingRules.push(lang === 'ar' ? t('auto.cognitiveEngine_personalVoice.3.648a9ab7', 'ar') : "Provide a comprehensive report divided into paragraphs and subheadings.");
  }

  if (preferences.wantsData) {
    formattingRules.push(lang === 'ar' ? t('auto.cognitiveEngine_personalVoice.2.a9d501eb', 'ar') : "You must explicitly include data indices (GMI, CFI, HRI) in the analysis.");
  }
...
export function formatVoiceForPrompt(instructions: VoiceInstruction, language: 'ar' | 'en' = 'ar'): string {
  const header = language === 'ar' ? t('auto.cognitiveEngine_personalVoice.1.cf0a659d', 'ar') : "Custom Tone and Style Rules:";
